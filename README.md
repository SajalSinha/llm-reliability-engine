...
<i>This repository focuses on failure handling, not model quality.</i>

## What This Repo Does

Given a user question, the system:

1) Generates multiple LLM responses

2) Evaluates how confident, consistent, and uncertain the output looks

3) Verifies the answer semantically using an independent model

4) Classifies risk and applies a fallback if needed

Returns the final answer along with reliability metadata

<i>The goal is simple:
...
1) Confidence proxy (length + lexical diversity)

2) Self-consistency across multiple generations

3) Lexical entropy as an uncertainty indicator
...
a) The question is treated as the premise

b) The answer is treated as the hypothesis

c) The system checks whether the answer is entailed
